...
public struct GenerateCasesByIntegerEncoding {
    private var constructors: [MemberBlockItemSyntax]
...
extension GenerateCasesByIntegerEncoding {
    mutating func append(_ case: EnumCaseElementSyntax) {
    ...
    consuming func block(extending subject: some TypeSyntaxProtocol) -> ExtensionDeclSyntax {
        .init(
            extendedType: subject,
...
extension GenerateCasesByIntegerEncoding: ExtensionMacro {
    public static func expansion(
        ...
        for cases: MemberBlockItemSyntax in cases.memberBlock.members {
            guard
            let cases: EnumCaseDeclSyntax = .init(cases.decl) else {
                continue
            }
