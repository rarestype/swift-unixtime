...
@frozen public struct DurationFormat: Equatable, Sendable {
    public var magnitude: UInt64
    /// This is packed below ``magnitude`` to improve memory layout.
    public var negative: Bool
    public var unit: Unit

    @inlinable public init(negative: Bool, magnitude: UInt64, unit: Unit) {
...
extension DurationFormat {
    private init(negative: Bool, attoseconds: UInt64) {
    ...
    private init(negative: Bool, seconds: UInt64) {
        let magnitude: UInt64
        let unit: Unit
...
extension DurationFormat {
    ...
    public init(_ nanoseconds: Nanoseconds) { self.init(Duration.init(nanoseconds)) }
    public init(_ microseconds: Microseconds) { self.init(Duration.init(microseconds)) }
    public init(_ milliseconds: Milliseconds) { self.init(Duration.init(milliseconds)) }

    public init(_ seconds: Seconds) {
        if      seconds.count < 0 {
    ...
    public init(_ minutes: Minutes) { self.init(Seconds.init(minutes)) }
    public init(_ hours: Hours) { self.init(Seconds.init(hours)) }
    public init(_ days: Days) { self.init(Seconds.init(days)) }
}
extension DurationFormat {
