...
extension QuantizedDuration {
    @inlinable public static func < (a: Self, b: Self) -> Bool { a.count < b.count }
}
extension QuantizedDuration {
    @inlinable public static var zero: Self { .init(count: 0) }

    @inlinable public static var max: Self { .init(count: .max) }

    @inlinable public static prefix func - (self: Self) -> Self { .init(count: -self.count) }

    @inlinable public static func + (a: Self, b: Self) -> Self {
        .init(count: a.count + b.count)
    }
...
extension QuantizedDuration {
    @inlinable public static func / (a: Self, b: Int) -> Self {
        .init(count: a.count / RawValue.init(b))
    }
    ...
    @inlinable public static func / (a: Self, b: Self) -> Double {
        Double.init(a.count) / Double.init(b.count)
    }
}
