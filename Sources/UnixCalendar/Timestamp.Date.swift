...
extension Timestamp {
    @frozen public struct Date: Equatable, Hashable, Sendable {
        ...
        @inlinable public init(year: Year, month: Month = .january, day: Int32 = 1) {
            self.year = year
            self.month = month
            self.day = day
        }
...
extension Timestamp.Date {
    ...
    @inlinable public var predecessor: Self { self.adjacent.before }

    /// A shorthand for `self.adjacent.after`. If you need both dates, it is more efficient to
    /// call `self.adjacent` and destructure the result.
    @inlinable public var successor: Self { self.adjacent.after }

    /// Computes the dates adjacent to this one. Performs no system calls.
    @inlinable public var adjacent: (before: Self, after: Self) {
...
extension Timestamp.Date: CustomStringConvertible {
    ...
    @inlinable public var description: String { self.description(separator: "-") }
}
...
extension Timestamp.Date {
    public init?(_ string: String, separator: Character) {
        self.init(string[...], separator: separator)
    }

    public init?(_ string: Substring, separator: Character = "-") {
        guard
        ...
        guard
        let day: Int32 = .init(string[day...]) else {
            return nil
        }
    ...
    @inlinable public func description(separator: Character) -> String {
        "\(self.year)\(separator)\(self.mm)\(separator)\(self.dd)"
    }
}
extension Timestamp.Date {
    @inlinable public var yyyymmdd: String { "\(self.year)\(self.mm)\(self.dd)" }

    @inlinable public var mm: String { "\(self.month.padded)" }

    @inlinable public var dd: String { self.day < 10 ? "0\(self.day)" : "\(self.day)" }
}
extension Timestamp.Date {
    /// Same as calling ``Month/short`` on ``month``.
    @inlinable public var mon: String { self.month.short }

    /// Same as calling ``Month/long(_:capitalized:)`` on ``month``.
    @inlinable public func month(_ locale: ISO.Locale) -> String { self.month.long(locale) }

    /// Formats the date in short form (`Jan 1, 1959`) in the given locale if supported, in
    /// English otherwise.
    @inlinable public func short(_ locale: ISO.Locale) -> String {
    ...
    @inlinable public func long(_ locale: ISO.Locale) -> String {
        switch locale.language {
