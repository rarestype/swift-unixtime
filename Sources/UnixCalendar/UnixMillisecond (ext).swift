...
extension UnixMillisecond {
    @inlinable public init?(
        ...
        guard
        let attosecond: UnixAttosecond = .init(utc: timestamp, sanity: checks) else {
            return nil
        }
    ...
    public static func now() -> Self { .init(truncating: .now()) }

    public var timestamp: Timestamp? { .init(secondSinceEpoch: Int.init(self.index / 1_000)) }
}
