...
@frozen public struct Timestamp: Equatable, Hashable, Sendable {
    public let weekday: Weekday
    public let date: Date
    public let time: Time

    @inlinable public init(weekday: Weekday, date: Date, time: Time) {
...
extension Timestamp {
    @inlinable public var components: Components { .init(date: self.date, time: self.time) }
}
extension Timestamp {
    /// Formats this timestamp as an HTTP GMT date.
    @inlinable public var http: String {
        """
...
extension Timestamp {
    init?(secondSinceEpoch: Int) {
        ...
        let month: Timestamp.Month = .init(rawValue: segmented.tm_mon + 1) else {
            return nil
        }
        ...
        self.init(
            weekday: weekday,
            date: .init(
                year: .init(rawValue: segmented.tm_year + 1900),
                month: month,
